...
class FDConfig(_BaseConfig):
    ...
    Parameters:
        ...
        scale_rewards (`str`, *optional*, defaults to `"group"`):
            Specifies the scaling strategy for rewards. Supported values are: `'group'` (default): rewards
            ...
            `'none'`: no scaling is applied. The Dr. GRPO paper recommends not scaling rewards, as scaling
    ...
    scale_rewards: str = field(
        default="group",
        metadata={
            "help": "Specifies the scaling strategy for rewards. Supported values are: `'group'` (default): rewards are scaled by the standard deviation within each group, ensuring unit variance within a group. `'batch'`: rewards are scaled by the standard deviation across the entire batch, as recommended in the PPO Lite paper. `'none'`: no scaling is applied. The Dr. GRPO paper recommends not scaling rewards, as scaling by the standard deviation introduces a question-level difficulty bias."
        },
    )
